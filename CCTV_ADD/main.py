...
# uvicorn main:app --reload로 실행(--reload 수정후 재실행)
...
def detect_objects(image: Image):
    ...
    for result in results:
        ...
        for box, confidences, class_id in zip(boxes, confidences, class_ids):
            ...
            cv2.putText(img,f'{label}{confidences:.2f}',(x1,y1),cv2.FONT_HERSHEY_SIMPLEX,0.9,(0,0,255),2)
...
async def detect_service(message: str = Form(...), file: UploadFile = File(...)):
    ...
    return DetectionResult(message=img_str, image=img_str)

    # 결론 : http://localhost:8000/detect 경로에 post 요청 처리
